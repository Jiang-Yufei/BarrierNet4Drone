...
import numpy as np
from Dataloader import test_solver as solver
...
class BarrierNet(nn.Module):
    ...
    def forward(self, x, sgn, distance):
        nBatch = x.size(0)
        ...
        # BarrierNet
        x = self.dCBF(distance, x0, x31, x32, sgn, nBatch)
               
        return x
    
    def dCBF(self, distance, x0, x31, x32, sgn, nBatch):
        ...
        barrier = dx**4 + dy**4 + dz**4 - self.R**4
        barrier_dot = 4*dx**3*vx + 4*dy**3*vy + 4*dz**3*vz
        Lf2b = 12*(px - self.obs_x)**2*vx**2 + 12*(py - self.obs_y)**2*vy**2 + 12*(pz - self.obs_z)**2*vz**2
        LgLfbu1 = torch.reshape(4*dx**3, (nBatch, 1))
        LgLfbu2 = torch.reshape(4*dy**3, (nBatch, 1))
        LgLfbu3 = torch.reshape(4*dz**3, (nBatch, 1))
